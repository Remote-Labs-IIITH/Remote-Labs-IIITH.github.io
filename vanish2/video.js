...
const paste = "https://www.youtube.com/embed/ZvVYw05tKGg"

// Get document refernce
const docRef = doc(db, 'Video_Id', "Vanish");
...
getDoc(docRef)
    .then((doc) => {
       ...
       // var url = start.concat(data_id).concat("&key=AIzaSyAwV-ZILe45AFhNwWcQ44p9h-42roXTnfg"); //streaming key
       var url = start.concat(data_id).concat("&key=ZvVYw05tKGg");
        ...
        fetch(url)
            ...
            .then((data1) => {
                ...
                if (data1.items[0].snippet.liveBroadcastContent == "live") {
                    document.getElementById('live').src = paste.concat(data_id).concat("?autoplay=1");
                    ...
                    fetch("https://youtube.googleapis.com/youtube/v3/search?channelId=UCOnjZcWuWP5gVdfbak1FJqw&eventType=live&type=video&key=ZvVYw05tKGg")
                    .then(response => response.json())
                        .then((data2) => {
                            const id = data2.items[0].id.videoId;
                            document.getElementById('live').src = paste.concat(id).concat("?autoplay=1");
