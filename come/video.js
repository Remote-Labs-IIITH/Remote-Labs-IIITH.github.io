...
const docRef = doc(db, 'Video_Id', "COME");
        ...
        fetch(url)
            ...
            .then((data1) => {
                ...
                if (data1.items[0].snippet.liveBroadcastContent == "live") {
                    ...
                    fetch("https://youtube.googleapis.com/youtube/v3/search?channelId=UCJZvt018fui20FoZDdUPG-A&eventType=live&type=video&key=AIzaSyAwV-ZILe45AFhNwWcQ44p9h-42roXTnfg")
                        .then(response => response.json())
                        .then((data2) => {
...
// Get document refernce
const docRef7 = doc(db, 'Video_Id', "COME1");

getDoc(docRef7)
    .then((doc) => {
        // console.log(doc.data(), doc.id);

        const data_id2 = doc.data().id;
        var url = start.concat(data_id2).concat("&key=AIzaSyAwV-ZILe45AFhNwWcQ44p9h-42roXTnfg");
        console.log(url);
        ...
        fetch(url)
            ...
            .then((data1) => {
                ...
                if (data1.items[0].snippet.liveBroadcastContent == "live") {
                    document.getElementById('live2').src = paste.concat(data_id2).concat("?autoplay=1");
                    
                } else {
                    fetch("https://youtube.googleapis.com/youtube/v3/search?channelId=UCoeuMuEC_9RL-Ydf-8EzzoA&eventType=live&type=video&key=AIzaSyAwV-ZILe45AFhNwWcQ44p9h-42roXTnfg")
                        .then(response => response.json())
                        .then((data2) => {
                            const id = data2.items[0].id.videoId;
