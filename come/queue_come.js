...
const collRef = collection(db, 'Queue_COME');
...
function myTimer1() {
    ...
    fetch('https://blynk.cloud/external/api/get?token=Ox9y5Ci8n4t0uMC-q9HCEq1o_p2obdg4&v21')
            ...
            if (myJSON == "0" && Number(document.getElementById('que_pos').innerHTML) == 1) {
                deleteDoc(doc(db, "Queue_COME", uid))
                    .then(() => {
                        ...
                        fetch("https://blynk.cloud/external/api/update?token=Ox9y5Ci8n4t0uMC-q9HCEq1o_p2obdg4&v21=1")
                    });
