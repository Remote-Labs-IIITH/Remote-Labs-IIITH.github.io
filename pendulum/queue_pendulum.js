...
const collRef = collection(db, 'Queue_PENDULUM');
...
var Free_or_not = setInterval(myTimer1, 4000);
function myTimer1() {
    //document.querySelector('form').onsubmit = function () {
    fetch('https://blynk.cloud/external/api/get?token=ePUMpo-rDkt1peBekOxcVgQpW9zebvfL&v5')
        ...
        .then(data => {
            ...
            if (myJSON == "0" && Number(document.getElementById('que_pos').innerHTML) == 1) {
                deleteDoc(doc(db, "Queue_PENDULUM", uid))
                    .then(() => {
                        location.href = 'pendu.html';
                        fetch("https://blynk.cloud/external/api/update?token=ePUMpo-rDkt1peBekOxcVgQpW9zebvfL&v5=1")
                    });
...
window.addEventListener('beforeunload',  (e) => {
    ...
    // fetch('https://blynk.cloud/external/api/update?token=Ox9y5Ci8n4t0uMC-q9HCEq1o_p2obdg4&v10=0')
